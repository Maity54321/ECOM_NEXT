...
import { getFilterConfig, getProducts } from "@/services/productService";
...
const AllProducts = () => {
  ...
  useEffect(() => {
    setLoading(true);
    setCurrentPage(1);
    fetchFilterConfig().then(() => {
      price[0] != undefined && price[1] != undefined && fetchProducts(1, false);
    });
  }, [keyword, category, recordsPerPage]);
